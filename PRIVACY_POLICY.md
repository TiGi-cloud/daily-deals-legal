# Privacy Policy for Daily Deals

**Last Updated:** [INSERT DATE]

**Company Name:** [YOUR COMPANY NAME]  
**App Name:** Daily Deals  
**Contact Email:** [YOUR EMAIL]  
**Website:** [YOUR WEBSITE]

---

## Introduction

This Privacy Policy describes how Daily Deals ("we", "our", or "us") collects, uses, and shares information when you use our Shopify application.

By installing and using Daily Deals, you agree to the collection and use of information in accordance with this policy.

---

## Information We Collect

### 1. Information from Shopify

When you install Daily Deals, we receive and store certain information from your Shopify store through the Shopify API:

**Store Information:**
- Store domain and name
- Store owner contact information
- Plan details

**Product Information:**
- Product titles, descriptions, and IDs
- Product types and tags
- Collection information
- Product pricing (for discount calculations)

**Discount Information:**
- Discount configurations you create
- Scheduled discount settings
- Discount performance data

### 2. Information You Provide

**Configuration Data:**
- Discount schedules you set up
- Product selections
- Collection preferences
- Scheduling intervals

### 3. Automatically Collected Information

**Usage Data:**
- App interaction logs
- Error reports
- Performance metrics
- Session information

**Technical Information:**
- IP address
- Browser type
- Device information
- Access times

---

## How We Use Your Information

We use the collected information for the following purposes:

### Primary Functions:
1. **Provide App Services:** Create and manage scheduled discount collections
2. **Process Data:** Calculate and apply discounts to selected products
3. **Schedule Automation:** Execute scheduled product rotations
4. **Store Operations:** Maintain and update your discount collections

### Secondary Functions:
1. **Improve Service:** Analyze usage to enhance app features
2. **Technical Support:** Diagnose and fix technical issues
3. **Communication:** Send app-related notifications and updates
4. **Compliance:** Meet legal and regulatory requirements

---

## Data Storage and Security

### Storage Location
Your data is stored securely in [SPECIFY REGION - e.g., "United States", "European Union", etc.] using industry-standard PostgreSQL databases.

### Security Measures
We implement appropriate technical and organizational measures including:

- **Encryption:** Data in transit is encrypted using TLS/SSL
- **Access Control:** Restricted access to personal data
- **Authentication:** Secure authentication using Shopify OAuth
- **Monitoring:** Regular security audits and monitoring
- **Backups:** Regular automated backups

### Data Retention
We retain your data for as long as:
- You have the app installed, OR
- As required by law or business purposes

After app uninstallation, your data is automatically deleted within [SPECIFY TIMEFRAME - e.g., "30 days"].

---

## Data Sharing and Disclosure

We do NOT sell your personal information. We may share information in the following circumstances:

### Shopify Platform
We share necessary data with Shopify to provide app functionality through the Shopify API.

### Service Providers
We may share data with trusted third-party service providers who assist us in:
- Database hosting ([SPECIFY PROVIDER])
- Analytics ([SPECIFY IF ANY])
- Error monitoring ([SPECIFY IF ANY])

All service providers are contractually obligated to protect your data.

### Legal Requirements
We may disclose information if required by law or in response to:
- Legal process (subpoenas, court orders)
- Protect rights and safety
- Investigate fraud or security issues

---

## Your Rights

### GDPR Rights (EU Users)
If you are located in the European Union, you have the following rights:

1. **Access:** Request copies of your personal data
2. **Rectification:** Request correction of inaccurate data
3. **Erasure:** Request deletion of your data ("right to be forgotten")
4. **Restriction:** Request limited processing of your data
5. **Portability:** Request transfer of your data
6. **Objection:** Object to processing of your data

### CCPA Rights (California Users)
If you are a California resident, you have the right to:

1. Know what personal information is collected
2. Know if personal information is sold or disclosed
3. Opt-out of sale of personal information
4. Request deletion of personal information
5. Non-discrimination for exercising your rights

### Exercising Your Rights
To exercise any of these rights, contact us at: [YOUR EMAIL]

We will respond to verified requests within 30 days.

---

## Cookies and Tracking

### Session Cookies
We use essential cookies to:
- Maintain your login session
- Remember your preferences
- Ensure app functionality

### Analytics (if applicable)
[IF USING ANALYTICS, DESCRIBE HERE]
We use [ANALYTICS PROVIDER] to understand app usage. You can opt-out by [METHOD].

---

## Children's Privacy

Daily Deals is not intended for users under 16 years of age. We do not knowingly collect information from children under 16.

If you believe we have collected information from a child under 16, please contact us immediately.

---

## International Data Transfers

If you are accessing Daily Deals from outside [YOUR COUNTRY], your data may be transferred to and stored in [YOUR COUNTRY].

We ensure appropriate safeguards are in place for international transfers, including:
- Standard Contractual Clauses (where applicable)
- Privacy Shield certification (if applicable)

---

## Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of changes by:
- Posting the new Privacy Policy in the app
- Updating the "Last Updated" date
- [OPTIONALLY] Sending an email notification

Continued use of the app after changes constitutes acceptance of the updated policy.

---

## Third-Party Links

Daily Deals may contain links to third-party websites. We are not responsible for the privacy practices of these websites.

---

## Contact Us

If you have questions about this Privacy Policy, please contact us:

**Email:** [YOUR EMAIL]  
**Address:** [YOUR BUSINESS ADDRESS]  
**Phone:** [YOUR PHONE NUMBER - OPTIONAL]

For data protection inquiries from EU users, contact our Data Protection Officer at: [DPO EMAIL - IF REQUIRED]

---

## Shopify-Specific Information

### Shopify's Privacy Policy
This app also operates under Shopify's Privacy Policy. Review it at: https://www.shopify.com/legal/privacy

### App Scopes
Daily Deals requests the following Shopify permissions:
- `read_products` - To access your product catalog
- `read_product_listings` - To access product collections
- `write_products` - To update product tags for discounts
- `write_discounts` - To create and manage discount codes

### Data Access
We only access data necessary for app functionality. We do not access:
- Customer personal information
- Payment information
- Order details
- Shipping addresses

---

## Compliance

Daily Deals is committed to compliance with:
- **GDPR** (General Data Protection Regulation)
- **CCPA** (California Consumer Privacy Act)
- **PIPEDA** (Personal Information Protection and Electronic Documents Act - Canada)
- **Shopify App Store Requirements**

---

**This is a template. Please:**
1. Replace all [BRACKETED] items with your actual information
2. Have a lawyer review this document
3. Update based on your actual data practices
4. Host this on your website
5. Keep it updated as your practices change
